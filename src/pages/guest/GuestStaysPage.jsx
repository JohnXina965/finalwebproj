...
function GuestHomePage() {
  ...
  const filteredAccommodations = useMemo(() => {
    ...
    let filtered = featuredAccommodations.filter(stay => {
      ...
      // Rating filter
      const matchesRating = searchFilters.minRating > 0 
        ? (stay.rating || 0) >= searchFilters.minRating 
        : true;
      
      // Property type filter
      const matchesPropertyType = searchFilters.propertyType
        ? (stay.type || '').toLowerCase() === searchFilters.propertyType.toLowerCase()
        : true;
      
      // Amenities filter
      const matchesAmenities = searchFilters.amenities.length > 0
        ? searchFilters.amenities.every(amenity => 
            (stay.features || []).some(feature => 
              feature.toLowerCase().includes(amenity.toLowerCase())
            )
          )
        : true;
      
      return matchesLocation && matchesGuests && matchesPrice && matchesRating && matchesPropertyType && matchesAmenities;
    });
