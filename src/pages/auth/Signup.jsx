...
const Signup = () => {
  ...
  const { checkEmailExists, checkEmailAuthMethod } = useAuth();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Check if email already exists and what authentication method it uses
      console.log('Checking if email exists:', formData.email);
      const authMethods = await checkEmailAuthMethod(formData.email);
      console.log('Email auth methods:', authMethods);
      
      if (authMethods.length > 0) {
        if (authMethods.includes('google.com')) {
          setMessage('❌ This email is already registered with Google. Please use "Sign in with Google" on the login page instead.');
